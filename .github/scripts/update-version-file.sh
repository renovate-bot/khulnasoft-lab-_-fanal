...
if [[ "$(curl -SsL https://api.github.com/repos/khulnasoft-lab/${BIN}/releases/tags/${VERSION} | jq .prerelease)" == "true" ]] ; then
...
aws s3 cp "${VERSION_FILE}" s3://toolbox-data.khulnasoft-lab.io/${BIN}/releases/latest/VERSION